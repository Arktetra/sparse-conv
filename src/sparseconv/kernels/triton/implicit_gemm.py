...
def sparse_submanifold_conv_fwd_implicit_gemm_kernel(
    ...
    num_pid_K = tl.cdiv(Ci, BLOCK_SIZE_K)
    ...
    for k in range(num_pid_K * V):
        v = k // num_pid_K
        bk = k % num_pid_K

        offset_neighbor_M = tl.load(neighbor_ptr + offset_M * V + v)
        input_ptrs = input_ptr + bk * BLOCK_SIZE_K + (offset_neighbor_M[:, None].to(tl.int64) * Ci + offset_K[None, :])
        neighbor_mask = offset_neighbor_M != 0xffffffff
        k_mask = offset_K < Ci - bk * BLOCK_SIZE_K
        ...
        weight_ptrs += min(BLOCK_SIZE_K, Ci - bk * BLOCK_SIZE_K)
    ...
    output_mask = (output_offset_M[:, None] < M) & (output_offset_Co[None, :] < Co)
...
def sparse_submanifold_conv_fwd_implicit_gemm(
    ...
    grid = lambda META: (
        ...
        * triton.cdiv(Co, META["BLOCK_SIZE_Co"]), 
    )
